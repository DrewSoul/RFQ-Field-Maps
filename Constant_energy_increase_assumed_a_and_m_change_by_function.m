...
CL = lambda*sqrt(1-((mass/(InitialEnergy+mass))^2));    %modulation period length (Initial) in m
TL = CL;    %Length along accelerator
%Modulation
m = zeros(x1,y1,z1);
...
%Aperture 
a = zeros(x1,y1,z1);
ai = 2/1000;    %Initial aperture in m
az = ai;
...
G = ((besseli(0,k.*a))+(besseli(0,k.*m.*a)))./(((m.^2).*(besseli(0,k.*a)))+(besseli(0,m.*k.*a)));    %Focusing efficiency
