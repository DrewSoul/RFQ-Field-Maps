...
mass = 938.28*10^6;    %Mass of particles in beam (Proton)
initialenergy = 20*10^3;     %Initial kinetic energy of particles
finalenergy = 2*10^6;    %Desired kinetic energy
        ...
        for j = 1:y1
            for i = 1:x1
                if (z(i,j,l) > TL-CL) && (z(i,j,l) <= TL)
                    ...
                    a(i,j,l) = az;
                elseif (z(i,j,l) <= 1)
                    deltaa = -(2*(z(i,j,l).^3)-(z(i,j,l).^2))/1000;
                    deltam = 1.8*z(i,j,l);
                    CL = CL + deltaCL;
                    mz = mi + deltam;
                    az = ai + deltaa;
